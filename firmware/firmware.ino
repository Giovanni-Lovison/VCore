...
//                                   SMBUS UNLOCK CODE  LOCK CODE
constexpr DeviceConfig UP9512_CFG = {0x39, 0x94, 0x87};

static const KnownDevice KNOWN_DEVICES[] = {
    {0x24, "uP9512", true, &UP9512_CFG},
    {0x25, "uP9512", true, &UP9512_CFG},
    {0x26, "uP9512", true, &UP9512_CFG},
    {0x27, "uP9512", true, &UP9512_CFG},
    {0x20, "uP9512", true, &UP9512_CFG}
};

const KnownDevice* get_device_info(uint8_t addr7) {
    for(const auto& dev : KNOWN_DEVICES) {
        if(addr7 == dev.addr) return &dev;
    }
    return nullptr;
}
...
enum SystemState {
    STATE_ERROR = 0xFF,
    STATE_LOCKED = UP9512_CFG.lock_code,
    STATE_UNLOCKED = UP9512_CFG.unlock_code
};
    ...
    for(uint8_t addr = 1; addr < 127; addr++) {
        ...
        if(Wire1.endTransmission() == 0) {
            if(num_devices < 16) {
                ...
                devices[num_devices].name = get_device_name(addr);
                num_devices++;
            }
        }
    }
...
const char* get_device_name(uint8_t addr) {
    const KnownDevice* dev = get_device_info(addr);
...
bool handle_bulk_reads(const JsonArray& reads, JsonDocument& res, uint8_t addr, uint32_t& op_count) {
    ...
    for(size_t i = 0; i < count; i++) {
        ...
        if(buffer[i] == SystemState::STATE_ERROR) {
            return false;
        }
    }
...
uint8_t getSMBusState() {
    if(!currentDevice.needs_bypass || !currentDevice.config) return 0xFF;
    return i2c_readRegister(currentDevice.addr, currentDevice.config->smbus_lock_reg);
}

bool setSMBusState(uint8_t state) {
    if(!currentDevice.needs_bypass || !currentDevice.config) return false;
    return i2c_writeRegister(currentDevice.addr, currentDevice.config->smbus_lock_reg, state) == SUCCESS;
}
        ...
        for(const auto& known : KNOWN_DEVICES) {
            if(known.needs_bypass) {
                ...
                if(Wire1.endTransmission() == 0) {
                    ...
                    currentDevice.needs_bypass = known.needs_bypass;
                    currentDevice.config = known.config;
                    break;
                }
            }
        }
...
// === SETUP ===
void setup() {    
    Serial.begin(115200);                           // Display
    Wire1.begin(I2C_DEV_SDA, I2C_DEV_SCL, 400000);  // I2C
    initDisplay();
    
    if (detectDevice()) {
        i2c_resetBus();
        
        uint8_t currentState = getSMBusState();
        if (currentState == SystemState::STATE_UNLOCKED) {
            showMessage("ALREADY", 2);
            display.setTextSize(1);
...
void unlockSMBus() {
    ...
    showMessage("UNLOCKING...", 2);
    
    bool unlockSuccess = false;
    for (int i = 0; i < 3 && !unlockSuccess; i++) {
        if (setSMBusState(UP9512_CFG.unlock_code)) {
            unlockSuccess = getSMBusState() == UP9512_CFG.unlock_code;
        }
    }
    ...
    display.printf("Unlocks: %d", successfulUnlocks);
    display.display();
    delay(1000);
}
